...
COPY --chown=avalon:avalon server.py game_logic.py ./
...
CMD ["gunicorn", "--worker-class", "gthread", "--workers", "1", "--threads", "100", "--bind", "0.0.0.0:5001", "--access-logfile", "-", "--error-logfile", "-", "server:app"]
